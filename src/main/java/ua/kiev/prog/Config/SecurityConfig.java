...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                ...
                .antMatchers("/usersForAdmin").hasRole("ADMIN") // Доступ на переход по URL "/admin" имеет только Администратор
                .antMatchers("/register").permitAll()
                .and()
                .exceptionHandling()
                .accessDeniedPage("/")  // В случае если USER пытаеться зайти по URL "/admin" его перекидывает на URL "/unauthorized"
                .and()
                .formLogin()
                .loginPage("/login")
